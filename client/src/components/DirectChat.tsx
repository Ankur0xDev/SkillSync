...
const DirectChat: React.FC = () => {
  ...
  return (
            <div className={`flex flex-col h-screen max-h-[80vh] ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-lg`}>
      <div className="p-4 border-b flex items-center space-x-3">
        ...
        <span className={`font-semibold ${theme === 'dark' ? 'text-gray-100' : 'text-gray-900'}`}>{otherUser?.name || 'User'}</span>
      </div>
      <div className="flex-1 overflow-y-auto p-4 space-y-2">
        {chat.messages.map((msg) => (
          ...
          >
            <div
              className={`px-4 py-2 rounded-lg max-w-xs break-words shadow text-sm ${theme === 'dark' ? 'bg-gray-700 text-gray-100' : 'bg-gray-200 text-gray-900'} ${
      ...
      <form onSubmit={handleSend} className="p-4 border-t flex space-x-2">
        <input
          type="text"
          className={`flex-1 border rounded-lg px-3 py-2 focus:outline-none ${theme === 'dark' ? 'bg-gray-700 text-gray-100' : 'bg-gray-200 text-gray-900'}`}
        ...
        <button
          type="submit"
          className={`${theme === 'dark' ? 'bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 disabled:opacity-50' : 'bg-purple-600 text-white px-4 py-2 rounded-lg hover:bg-purple-700 disabled:opacity-50'}`}
