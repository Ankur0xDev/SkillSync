      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {statCards.map((stat, index) => {
            ...
            return (
              ...
              >
                <Link
                  to={stat.link}
                  className={`block ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} rounded-2xl shadow-lg p-6 hover:shadow-xl transition-shadow duration-300`}
                >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            >
              <h2 className={`text-2xl font-bold ${theme === 'dark' ? 'text-gray-100' : 'text-gray-900'} mb-6`}>Quick Actions</h2>
                ...
                {quickActions.map((action) => {
                  ...
                  return (
                    <Link
                      key={action.title}
                      to={action.link}
                      className={`p-6 ${theme === 'dark' ? 'border border-gray-700' : 'border border-gray-200'} rounded-xl hover:border-purple-300 hover:shadow-md transition-all duration-200 group`}
                    >
                      ...
                      <h3 className={`font-semibold ${theme === 'dark' ? 'text-gray-100' : 'text-gray-900'} mb-2`}>
                        {action.title}
                      </h3>
          ...
          <div className="space-y-8">
            ...
            >
              <h3 className={`text-lg font-semibold ${theme === 'dark' ? 'text-gray-100' : 'text-gray-900'} mb-4`}>
                Pending Requests
              </h3>
                  ...
                  {pendingRequests.map((request) => (
                    ...
                    >
                      <div className="flex items-center space-x-3 mb-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <h4 className={`font-medium ${theme === 'dark' ? 'text-gray-100' : 'text-gray-900'} text-sm`}>
                            {request.name}
                          </h4>
                          {request.bio && (
                      ...
                      <div className="flex space-x-2">
                        <button
                          ...
                          className={`flex-1 ${theme === 'dark' ? 'bg-green-600 text-white' : 'bg-purple-600 text-white'} py-2 px-3 rounded-lg text-sm hover:bg-purple-700 transition-colors`}
                        >
                          Accept
                        </button>
                        <button
                          onClick={() => rejectRequest(request._id)}
                          className={`flex-1 ${theme === 'dark' ? 'bg-gray-700 text-gray-100' : 'bg-gray-100 text-gray-700'} py-2 px-3 rounded-lg text-sm hover:bg-gray-200 transition-colors`}
                        >
                          Decline
                        </button>
                      </div>
            ...
            <motion.div
              ...
              className={`${theme === 'dark' ? 'bg-gradient-to-r from-green-600 to-blue-600' : 'bg-gradient-to-r from-purple-600 to-blue-600'} rounded-2xl shadow-lg p-6 text-white`}
            >
              <div className="flex items-center space-x-2 mb-4">
                <TrendingUp className="w-5 h-5" />
                <h3 className={`font-semibold ${theme === 'dark' ? 'text-gray-100' : 'text-gray-900'}`}>Boost Your Profile</h3>
              </div>
            ...
            >
              <div className="flex items-center space-x-2 mb-4">
                ...
                <h3 className={`font-semibold ${theme === 'dark' ? 'text-gray-100' : 'text-gray-900'}`}>Recent Activity</h3>
              </div>
              <div className="space-y-3">
                <div className="flex items-center space-x-3 text-sm">
                  <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                  <span className={`${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>Profile viewed {stats.profileViews} times</span>
                </div>
                <div className="flex items-center space-x-3 text-sm">
                  <div className="w-2 h-2 bg-blue-500 rounded-full"></div>
                  <span className={`${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>{stats.connections} active connections</span>
                </div>
                <div className="flex items-center space-x-3 text-sm">
                  <div className="w-2 h-2 bg-purple-500 rounded-full"></div>
                  <span className={`${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>{stats.sentRequests} requests sent</span>
                </div>
              </div>
            </motion.div>
          </div>
